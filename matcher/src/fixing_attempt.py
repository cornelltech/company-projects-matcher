...
def read_input(file, normalize=True):
	...
	#rearrange_spots(students_lst, projects, names_projects)
	...
	for p in projects:
		students = p.MBA_list + p.MEng_list
		if (p.ID in names_projects.keys() and len(students) > 0 and len(students) < 4):
			print names_projects[p.ID] 
	# Need to fix size issues first.
...
# 			if (cur_spot < len(ranks) - 1):
# 				cur_spot_proj_ID = ranks[cur_spot]
# 				next_spot_proj_ID = ranks[cur_spot + 1]
...
# 				next_project = get_project_from_ID(next_spot_proj_ID, projects)

# 				# Match to project with least empty spots.
# 				top_proj_empty_spots = top_project.remaining_MEng_spots + top_project.remaining_MBA_spots
# 				next_project_empty_spots = next_project.remaining_MEng_spots + next_project.remaining_MBA_spots

# 				if (top_proj_empty_spots < next_project_empty_spots):
# 					matched = top_project.add_student(cur_student)

# 				else:
# 					matched = next_project.add_student(cur_student)
...
def rearrange_spots(students_lst, projects, np = []):
	...
	print "The unfilled students are",
	for s in unfilled_students:
		print s.ID
	...
	for proj in projects:
		if (has_empty_spot(proj)):
			...
			for student in unfilled_students:
				...
				# TODO: fix problem here. None of our matchings are good in the beginning
				# so all of our students are being assigned to two teams.
